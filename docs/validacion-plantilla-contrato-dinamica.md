...
Tras corregir el runtime de Railway para usar `Chromium` del sistema, la misma prueba se ejecutó contra **https://app.evgreen.lat** y devolvió nuevamente la plantilla `id=5`, versión `2.3-dinamica`, con `contractCountBefore=0` y `contractCountAfter=0`, confirmando que la validación productiva tampoco crea expedientes. El PDF generado por producción pesó **222.294 bytes** y conservó el mismo hash lógico `56bfbfa87d840658d949254658606a68cbf38add04a16c301a513c12371cc2cd`. La revisión visual de sus páginas 1 a 3 confirmó portada, versión y tablas de partes correctas; la revisión de las páginas 15 y 16 confirmó anexos, un único bloque de firmas y pie de integridad sin superposición ni error 500.

La verificación productiva se ejecutó sobre los procedimientos protegidos reales con una sesión Admin temporal de cinco minutos y sin imprimir ni persistir el token. El navegador interactivo de QA no estuvo disponible para capturar la pantalla del panel, por lo que **no se declara una comprobación visual por clic de la UI**. Sí quedó comprobado el mismo backend que consume la interfaz: listado autenticado de cuatro versiones, presencia de la v2.3 `DRAFT`, tres espacios elegibles, generación PDF productiva y ausencia de mutaciones contractuales.
