...
## Validación productiva y guardado real

Después del despliegue en `app.evgreen.lat`, el asistente híbrido se validó contra los procedimientos protegidos reales. El análisis del DOCX v3.0 devolvió el mismo hash `435017f4627e830dea8c66e415023625b2e9c40c1a3e593e78d32cd3bea27165`, detectó los mismos ocho marcadores y propuso automáticamente las ocho asociaciones correctas. La vista previa volvió a exigir la huella de mapeo y mantuvo el bloqueo `412` cuando se intenta guardar sin la previsualización correspondiente.

Luego se ejecutó el guardado real de la versión jurídica `3.0` como borrador. La base activa registró la plantilla **id=22**, nombre **Contrato de concesión de sitio EDS**, estado **DRAFT**, con su `content_hash` original y el `mappingFingerprint` `b15c7f9bee3b6305fa1510f54af8540d76c0780a2a9ef12dd622ca5cf2e4c564`. El `variableSchema` persistido conserva las ocho asociaciones de alias a variables canónicas, por lo que la trazabilidad del mapeo queda almacenada junto con la versión.

Finalmente se probó la vista previa contractual productiva con esa nueva plantilla `id=22`. El backend generó `preview-contrato-v3-produccion.pdf` con **270.732 bytes**, contrato temporal `EVG-PREV-2026-75CF9B`, hash lógico `fe29bcf1a9a70ece92c8a325a0224c0637bed78c17cbb8ee253bc4335f315018` y **0 expedientes antes / 0 expedientes después**. La revisión visual de las páginas 1 a 4 confirmó que el espacio real **EDS Santa Ana** y los datos reales del aliado quedaron ubicados en sus celdas correctas, sin residuos `{{...}}`.
