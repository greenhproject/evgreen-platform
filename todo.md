...
- [x] Publicación crowdfunding QA/masivo: checkpoint `d8902cee`, rebase seguro de un único cambio sobre `github/main`, push no forzado `2881c239` y Railway `Success - app.evgreen.lat`. Producción conserva 27 proyectos, 26 participaciones por $1.895.000.000, 2 proyectos con recaudo por $1.100.000.000 y 6 vinculados a estación; registra 0 proyectos de los cinco fixtures exactos de `spaces.test.ts` y 0 del fixture `QA Bulk Crowdfunding Cleanup`. Los únicos `spaceSubmissionId` sin postulación son los dos casos ambiguos preservados deliberadamente (`120001` Villa yuliana y `150001` Conjunto de prueba). El bundle productivo contiene selección visible, cambio masivo de estado, confirmación, motivo de omitidos y `bulkManageProjects`; el endpoint sin sesión responde 401 ante una lista vacía, confirmando autorización. No se ejecutaron acciones destructivas sobre proyectos reales.

## Crowdfunding — edición financiera y fuente canónica de escenarios (2026-09-10)
- [x] Diagnóstico del bloqueo: al abrir un proyecto vinculado el cliente reemplazaba los valores actuales con el snapshot de Espacios y luego comparaba contra ese mismo snapshot; el backend veía cambios frente al registro vigente, pero el cliente no enviaba el motivo visible. Ahora el formulario abre valores actuales, compara contra el proyecto vigente y envía el motivo únicamente ante un cambio real, sin debilitar la auditoría.
- [x] Fuente canónica de escenarios: módulo compartido con waterfall `venta − energía − gastos fijos → aliado → neto → inversionista/EVGreen`, escenarios Pesimista 4 h/día, Realista 6 h/día y Optimista 9 h/día; el escenario seleccionado es la única fuente de ROI anual y payback en creación/edición, auto-creación/publicación desde Espacios y prospecto PDF.
- [x] Backend y persistencia: cálculo autoritativo en servidor, creación bloqueada sin escenario, recálculo obligatorio al cambiar inversión/potencia/ROI/payback, snapshot JSON versionado, escenario, fecha y responsable; publicación desde Espacios preserva ajustes vigentes y reemplaza los defaults legacy 85/14 por el escenario calculado; migración `0018_crowdfunding_financial_projection.sql` aplicada sin modificar proyectos reales.
- [x] Modal financiero mobile-first: valores actuales editables, herencia original separada para auditoría, ROI/payback de solo lectura, supuestos configurables, tres escenarios comparables, explicación de metodología y ancho/scroll responsive.
- [x] QA financiero: TypeScript limpio, pruebas focalizadas de edición/creación/escenarios/Espacios y suite completa 199 archivos/2.203 pruebas aprobadas tras integrar sin sobrescritura el desarrollo concurrente de cancelación/archivo seguro; build productivo exitoso. Render SSR confirmó los tres escenarios y clases responsive. Cero fixtures residuales, 4 columnas financieras y 3 columnas de cancelación verificadas, y proyecto Cunduy `210001` confirmado sin modificaciones durante QA.
- [x] Publicación financiera: checkpoint/GitHub `a5b4c7fc`, push fast-forward no forzado y Railway `Success - app.evgreen.lat`. El bundle productivo contiene ROI anual aplicado, payback calculado, motivo de ajuste y cancelación/archivo; `crowdfunding.updateProject` sin sesión responde 401. Logs posteriores sin errores de Crowdfunding, 4 columnas financieras y 3 de cancelación presentes, cero fixtures QA y proyecto real Cunduy `210001` preservado sin escrituras durante la comprobación.

## Prospectos de Espacios — coherencia de escenarios y legibilidad PDF (2026-09-14)
- [x] Auditar el PDF adjunto `SPE-2026-0067`: la aritmética original era internamente consistente, pero usaba costo de energía obsoleto de $700/kWh frente a $850/kWh vigente, denominaba de forma ambigua horas equivalentes y presentaba 240 kW proyectados ante 112 kVA declarados. El espacio `67` está marcado `requires_upgrade`; por tanto la rentabilidad no puede comunicarse como potencia actualmente habilitada.
- [x] Rastrear simulador → payload → prospecto PDF: el diálogo conservaba defaults aislados de $700/kWh y no transportaba gastos fijos ni condición eléctrica. El endpoint ahora resuelve tarifa, energía y eficiencia vigentes desde plataforma cuando no hay una excepción explícita.
- [x] Corregir cálculos, etiquetas y serialización: nuevo guardia compartido vincula escenario, transformer y viabilidad; bloquea emitir ROI/payback de una potencia sujeta a ampliación sin confirmación de CAPEX y nota técnica. El waterfall descuenta gastos fijos antes de aliado/reparto, conserva escenario 4/6/9 h equivalentes y compacta importes sin truncar.
...
- [x] Dictamen emitido: las cifras del modelo son conciliables y auditables bajo los supuestos impresos, pero el PDF recibido antes de esta última corrección no debe compartirse porque tenía divulgación técnica truncada y no identificaba el transformador propuesto. Apto únicamente después de regenerarlo desde la versión publicada, manteniendo los límites de proyección y respaldando CAPEX/transformador con soporte técnico y cotización.
...
- [x] Publicación técnica: checkpoint `1c5b5e15`, push no forzado a GitHub y Railway `Success - app.evgreen.lat`. Producción conserva plantilla ID `1586000253258258` en `IN_REVIEW`, cero alertas pendientes de plantilla y el endpoint de registro rechaza peticiones sin sesión (401); no se ejecutaron envíos reales durante la comprobación.
- [ ] Cuando Meta apruebe la plantilla, confirmar destinatario, contenido y que el conector siga disponible antes de enviar una prueba real autorizada por WhatsApp; comprobar la entrega en los logs por ID de mensaje sin exponer el teléfono.

## Scoring IA de Espacios — DC, prefactibilidad y reparto operativo (2026-09-18)
- [x] Sustituir el prompt genérico por una evaluación multimodal estructurada: utiliza las fotografías registradas (hasta ocho) para identificar únicamente evidencia visible de área útil, accesos, circulación, seguridad y elementos eléctricos; deja limitaciones expresas cuando el material no permite concluir.
- [x] Convertir la política técnica en regla determinista y reutilizable: EVGreen recomienda exclusivamente carga rápida DC desde 120 kW; un diseño de 120 kW exige transformador dedicado mínimo de 150 kVA con FP de 0,80; cualquier prospecto queda condicionado a acometida/ampliación, estudio y aprobación del operador de red, sin presumir capacidad libre de un transformador existente.
- [x] Incorporar scoring ponderado y auditable (técnica 25, tráfico 30, predio visual 20, contexto/demanda 15 y operación/acceso 10). Las EDS reciben una señal operativa mínima de tráfico, manteniendo el aforo físico como evidencia pendiente.
- [x] Aplicar la regla comercial en la proyección y el prospecto: EDS 60 % inversionista / 40 % EVGreen y otros negocios 70 % / 30 %, ambos sobre margen neto; la comisión del dueño del sitio continúa separada como costo del aliado sobre el margen bruto.
- [x] Mostrar el desglose, resultado visual, transformador requerido y política de reparto en Administración → Espacios; bloquear potencias inferiores a 120 kW en la edición y generación de prospectos.
- [x] Validación local inicial: TypeScript limpio; 39 pruebas focales de Espacios, herencia, escenarios, notas de red y nuevas políticas aprobadas. Pendiente: build completo, checkpoint, publicación no forzada a GitHub y verificación Railway.

## Reserva → inicio de carga → OCPP: asociación y liberación segura (2026-09-18)
- [x] Auditoría en vivo: la reserva #240001 quedó en `NO_SHOW` sin transacción y el EVSE #150001 permaneció `RESERVED`; se confirmó que el proceso de no-show generaba `UPDATE evses SET` vacío por escribir la propiedad inexistente `status` en vez de `connectorStatus`.
- [x] Recuperación puntual y condicionada: se liberó únicamente el bloqueo huérfano tras comprobar que no había reserva `ACTIVE` ni transacción `IN_PROGRESS`. El cargador reportó después `PREPARING`, por lo que no se ejecutó ninguna acción adicional sobre la sesión física.
- [x] Corregir ciclo de vida: la reserva sólo se marca `FULFILLED` y registra `transactionId` cuando OCPP confirma el inicio; nunca al pulsar “iniciar carga”. OCPP 1.6 y 2.0.1 resuelven titularidad por sesión pendiente persistida y luego por idTag/idToken; 2.0.1 ya no asigna el usuario fijo #1.
- [x] Proteger titularidad y estado: sólo el titular puede iniciar sobre `RESERVED`; el conector se resuelve por `connectorId` o `evseIdLocal`; futuras, vencidas, canceladas y cumplidas no bloquean. La proyección de EVSE conserva `connectorStatus` y metadatos de reserva correctos.
- [x] Sustituir escrituras directas e inválidas de los jobs de reserva/no-show por `updateEvseStatus` (fuente única, auditoría y sincronización). El job intenta `ReserveNow` con idTag del usuario y persiste `ocppReservationId` sólo con respuesta OCPP `Accepted`.
- [x] Validación: TypeScript limpio; 4 suites focales/64 pruebas, suite completa 206 archivos/2.229 pruebas y build productivo aprobados. Pendiente: checkpoint, publicación no forzada y comprobación Railway.

## Reservas activas y notificaciones WhatsApp — 2026-09-18

- Corregido el banner del mapa que leía el campo inexistente `status` aunque la fuente autoritativa persiste `reservationStatus`; ahora conserva la próxima reserva activa, incluso si es futura, y refresca cada 30 segundos.
- `Mis reservas` usa la estación enriquecida del backend (`stationName` y `stationAddress`), permite ir a la estación desde cada reserva y alinea la política visible de reembolso con el backend: 100% con 30 minutos o más; sin reembolso por debajo de ese umbral.
- Se implementó un ciclo de avisos interno e idempotente por reserva: confirmación, recordatorios 30/5 min, inicio real confirmado por OCPP 1.6/2.0.1, cancelación, ventana de gracia y no-show. Nunca se asocia una carga a una reserva antes de la confirmación física OCPP.
- WhatsApp usa la plantilla Utility `evgreen_reserva_actualizacion_v1` (ID `1635773261508729`), creada vía Meta API y actualmente `IN_REVIEW`. No se enviaron mensajes reales durante la implementación. Mientras Meta no apruebe, las alertas internas se conservan y el Heartbeat autenticado reintentará sólo los avisos sin `wamid` cuando la plantilla esté `APPROVED`.
...
## SOC AC adaptativo por recalibraciones — 2026-09-19
- [x] Auditada la sesión activa `1140025`: la recalibración de 79 % sí reinició su ancla en 14,974 kWh; el límite previo era que la capacidad de 60 kWh permanecía fija y por ello cada corrección posterior sólo reiniciaba el porcentaje, sin aprender de la diferencia observada en el vehículo.
- [x] Incorporar un modelo conservador por sesión AC. Cada nueva lectura real compara delta de kWh entregados y delta de SOC contra la última ancla; estima una capacidad energética efectiva y la actualiza gradualmente, con mínimos de evidencia, límites físicos, tope de paso y rechazo de observaciones atípicas. La ficha de capacidad declarada no se modifica.
...
## Corrección crítica — Push Android/iOS y trazabilidad de entrega (2026-09-19)
- [x] Auditoría: el backend Firebase está autenticado, pero la prueba anterior sólo reportaba éxito al aceptar el mensaje; no distinguía aceptación, recepción o apertura. El registro heredado `users.fcmToken` sólo conservaba un teléfono por usuario.
- [x] Servidor: se añadieron `push_devices` y `push_delivery_events` (migración aditiva `0046_push_device_delivery_observability.sql`, aplicada y verificada). Los estados son `REQUESTED`, `ACCEPTED`, `RECEIVED`, `OPENED` y `FAILED`; nunca se presenta un acuse FCM como entrega real. Se preservan los campos heredados para transición segura.
- [x] Cliente: Android/iOS registran plataforma y versión por dispositivo, sincronizan renovaciones de token, reportan `RECEIVED`/`OPENED` desde los eventos nativos y dejan un panel de estado verificable en Preferencias. El botón de prueba ahora dice “aceptada por el proveedor” hasta recibir el acuse de la app.
- [x] Enrutamiento: `sendUserPush` entrega a Web Push y a todos los FCM activos del usuario; alertas de saldo usan el unificador, sin excluir por el campo histórico nulo. Desactivar Push en un teléfono no borra los dispositivos del mismo usuario.
...
## Preferencias Push del perfil — emisión real y alcance (2026-09-19)
- [x] Auditoría: el canal Push y la prueba nativa están operativos; los tres interruptores de perfil se persistían. `Promociones y ofertas` ya se respetaba en broadcasts, pero `Carga completada` y `Saldo bajo` no estaban conectados de forma uniforme a los emisores.
- [x] Corrección: se creó una política central dentro del envío unificado. Al desactivar Carga completada se suprimen todos los eventos `charging_complete` (fin físico, batería llena, monto objetivo y período de gracia); al desactivar Saldo bajo se suprime únicamente `low_balance`. Alertas de seguridad —por ejemplo, detención por saldo cero—, pagos y errores permanecen activas para no ocultar un riesgo operacional.
- [x] Promociones queda filtrada dos veces: desde la audiencia de broadcast y desde la política central, evitando envíos directos accidentales. Usuarios previos sin campo conservan consentimiento por compatibilidad.
- [x] Transparencia: la pantalla especifica que la proximidad usa ubicación mientras la app está en uso. No se presenta como geofencing permanente en segundo plano; en app abierta se revisa al entrar en cualquier sección de usuario y luego cada cinco minutos, con cooldown de 30 min y filtros de compatibilidad/disponibilidad/precio.
- [x] Validación focal: TypeScript limpio y 27 pruebas Push aprobadas. Pendiente: suite completa, build, checkpoint, publicación y verificación Railway.

## Protección de reserva frente a carga anterior en curso (2026-09-22)
- [x] Se implementó la regla A: **nunca** se interrumpe una carga física ya iniciada para atender una reserva posterior. En cambio, antes de permitir una nueva carga, el planificador compara su duración estimada con la siguiente reserva del mismo conector y exige liberarlo 15 minutos antes de la hora reservada.
- [x] El control está en las dos capas: `validateAndEstimate` muestra en la app el tiempo máximo/alternativa antes de confirmar y `startCharge` vuelve a validarlo de forma autoritativa. Una carga ya iniciada no se corta; por tanto no se puede prometer una disponibilidad que ya no existe.
