...
export default function HostOccupancyLiquidations() {
  ...
  const pendingTransfer = (records ?? []).filter((r: any) => !r.allyPaidAt).reduce((acc: number, r: any) => acc + r.allyTransfer, 0);
  const paidTransfer = (records ?? []).filter((r: any) => !!r.allyPaidAt).reduce((acc: number, r: any) => acc + r.allyTransfer, 0);
