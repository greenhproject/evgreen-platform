...
export default function TechnicianStations() {
  ...
  const createStationMutation = trpc.stations.create.useMutation({
    onError: (error) => toast.error(`Error al crear estación: ${error.message}`),
  });
  ...
  const updateStationMutation = trpc.stations.update.useMutation({
    ...
    onError: (error) => toast.error(`Error al actualizar estación: ${error.message}`),
  });

  const createEvseMutation = trpc.evses.create.useMutation({
    onError: (error: any) => toast.error(`Error al crear conector: ${error.message}`),
  });

  const updateEvseMutation = trpc.evses.update.useMutation({
    onSuccess: () => { toast.success("Conector actualizado"); refetch(); },
    onError: (error: any) => toast.error(`Error al actualizar conector: ${error.message}`),
  });

  const deleteEvseMutation = trpc.evses.delete.useMutation({
    onSuccess: () => { toast.success("Conector eliminado"); refetch(); },
    onError: (error: any) => toast.error(`Error al eliminar conector: ${error.message}`),
  });
  ...
  const deleteStationMutation = trpc.stations.delete.useMutation({
    ...
    onError: (error) => toast.error(`Error al eliminar estación: ${error.message}`),
  });
  ...
  const isStationConnectedOCPP = (station: any) => {
    ...
    return ocppConnections.some((conn: any) => 
      conn.ocppIdentity === ocppId || conn.stationId === station.id
    );
  };
  ...
  const getConnectorSummary = (station: any) => {
    if (!station.evses || station.evses.length === 0) return "Sin conectores";
  ...
  const disconnectedCount = (stations?.filter(s => s.isActive).length || 0) - connectedCount;
  ...
  const handleUpdateStation = async () => {
    ...
    try {
      ...
      const newConns = connectors.filter(c => c.id.startsWith('conn-'));
      if (newConns.length > 0) {
        ...
        for (const connector of newConns) {
          for (let i = 0; i < connector.quantity; i++) {
            await createEvseMutation.mutateAsync({
        ...
        toast.success(`${newConns.reduce((acc, c) => acc + c.quantity, 0)} conector(es) agregado(s)`);
      }
    } catch (error) {
      // Error manejado en onError
    }
  ...
  // Formulario de estación (sin precios ni zonas premium)
    ...
    <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
              <div>
                ...
                <p className="text-xs text-muted-foreground">Habilitar o deshabilitar</p>
              </div>
              <Switch 
      ...
      {!isEdit && (
        <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-muted-foreground">
                        {connector.powerKw} kW x {connector.quantity} unidad(es)
                      </div>
                    </div>
                  </div>
                  <Button variant="ghost" size="icon" onClick={() => removeConnector(connector.id)}>
                    <X className="w-4 h-4" />
                  </Button>
                </div>
              ))}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 p-4 border border-dashed rounded-lg">
            ...
            <div className="flex items-end">
              <Button variant="outline" className="w-full" onClick={addConnector}>
                    ...
                    <AlertDialog>
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          ...
                          <AlertDialogDescription>
                            ¿Eliminar conector #{evse.evseIdLocal || index + 1} ({getConnectorLabel(evse.connectorType)})?
                          </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
      ...
      {/* Nota: el técnico no puede modificar precios ni tarifas */}
      <div className="p-3 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
        <p className="text-xs text-yellow-500">
          <strong>Nota:</strong> La configuración de precios y tarifas solo puede ser modificada por el administrador.
        </p>
      </div>
    </div>
  );
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Gestión técnica y monitoreo de cargadores
          </p>
        </div>
      ...
      <Card className="p-4">
        <div className="flex items-center gap-4">
          <div className="relative flex-1">
            ...
            <Input 
              placeholder="Buscar por nombre, dirección, ciudad o ID OCPP..." 
              className="pl-10"
              value={searchQuery}
                      ...
                      <div className="flex items-center justify-end gap-1">
                        ...
                        <Button 
                          ...
                          onClick={() => {
                            ...
                            navigate(`/technician/logs?filter=${encodeURIComponent(ocppId)}`);
                          }}
                        >
          ...
          {viewingStation && (
            <Tabs value={detailsTab} onValueChange={setDetailsTab} className="w-full">
              ...
              <TabsContent value="info" className="space-y-6 mt-4">
                ...
                {/* Conectores con estado OCPP real */}
                <div className="space-y-3">
                          ...
                          >
                            ...
                            <Badge className={
                              ...
                              {isAvailable ? 'Disponible' : 
                               isCharging ? 'Cargando' : 
                               isPreparing ? 'Preparando' : realStatus}
                            </Badge>
                          </div>
                        );
                      })}
                    ...
                    <Card className="bg-red-500/10 border-red-500/30">
                      <div className="p-4 flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">No hay conexión OCPP activa</p>
                        </div>
                      </div>
                    </Card>
                  );
