/**
 * Layout para el Aliado Comercial (dueño del espacio)
 * Similar al InvestorLayout pero con menú adaptado al rol host
 */
...
const hostMenuItems = [
  ...
  { icon: Building2, label: "Mis Espacios", path: "/host/spaces" },
...
const SIDEBAR_WIDTH_KEY = "host-sidebar-width";
...
export default function HostLayout({
        ...
        <div className="flex flex-col items-center gap-8 p-8 max-w-md w-full">
          <div className="flex flex-col items-center gap-4">
            ...
            <h1 className="text-2xl font-bold tracking-tight text-center">
              Portal Aliado Comercial
            </h1>
            <p className="text-sm text-muted-foreground text-center max-w-sm">
              Accede a tu panel para monitorear los ingresos generados por las estaciones de carga en tu espacio.
            </p>
          </div>
          <Button
            onClick={() => {
  ...
  return (
    ...
    >
      <HostLayoutContent setSidebarWidth={setSidebarWidth}>
        {children}
      </HostLayoutContent>
    </SidebarProvider>
  );
...
function HostLayoutContent({
  ...
  const activeMenuItem = hostMenuItems.find(item => item.path === location);
