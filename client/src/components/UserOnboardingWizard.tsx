...
// Select.Content renders in a portal. The onboarding mask has z-100, so its
// menus must deliberately sit above it to remain visible and touchable.
export const USER_ONBOARDING_SELECT_CONTENT_CLASS = "z-[110]";
    ...
    if (currentStep === 3) {
      return (
        <div className="space-y-5">
          ...
          <div className="grid gap-4 sm:grid-cols-2"><Field label="Marca"><Select value={brand} onValueChange={setBrand}><SelectTrigger><SelectValue placeholder="Selecciona una marca" /></SelectTrigger><SelectContent className={USER_ONBOARDING_SELECT_CONTENT_CLASS}>{VEHICLE_BRANDS.map((item) => <SelectItem key={item} value={item}>{item}</SelectItem>)}</SelectContent></Select></Field><Field label="Modelo"><Input value={model} onChange={(event) => setModel(event.target.value)} placeholder="Ej. iX3, EX30, e-208" /></Field></div>
          <div className="grid gap-4 sm:grid-cols-2"><Field label="Batería útil (kWh)" optional><Input type="number" min="1" max="999" value={batteryCapacityKwh} onChange={(event) => setBatteryCapacityKwh(event.target.value)} placeholder="Ej. 64" /></Field><Field label="Conector principal"><Select value={connectorType} onValueChange={setConnectorType}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent className={USER_ONBOARDING_SELECT_CONTENT_CLASS}>{connectorOptions.map((item) => <SelectItem key={item.value} value={item.value}>{item.label}</SelectItem>)}</SelectContent></Select></Field></div>
